...
export async function addBookmark(guideId: string): Promise<void> {
  await getDb().runAsync(
    'INSERT OR REPLACE INTO bookmarks (guide_id, saved_at) VALUES (?, ?)',
    [guideId, Date.now()],
  );
}
...
export async function getBookmarks(): Promise<string[]> {
  const rows = await getDb().getAllAsync<{ guide_id: string }>(
    'SELECT guide_id FROM bookmarks ORDER BY saved_at DESC',
  );
...
export async function recordViewed(guideId: string): Promise<void> {
  ...
  await database.runAsync(
    'INSERT OR REPLACE INTO recently_viewed (guide_id, viewed_at) VALUES (?, ?)',
    [guideId, Date.now()],
  );
  // Purge oldest beyond 20
  await database.runAsync(`
    DELETE FROM recently_viewed
    WHERE guide_id NOT IN (
      SELECT guide_id FROM recently_viewed ORDER BY viewed_at DESC LIMIT 20
    )
  `);
}
...
export async function setChecklistItem(
  ...
  await getDb().runAsync(
    `INSERT OR REPLACE INTO checklist_state (checklist_id, item_id, checked, updated_at)
     VALUES (?, ?, ?, ?)`,
    [checklistId, itemId, checked ? 1 : 0, Date.now()],
  );
}

export async function getChecklistState(checklistId: string): Promise<Record<string, boolean>> {
  const rows = await getDb().getAllAsync<{ item_id: string; checked: number }>(
    'SELECT item_id, checked FROM checklist_state WHERE checklist_id = ?',
    [checklistId],
  );
...
export async function getUserTier(): Promise<string> {
  const row = await getDb().getFirstAsync<{ tier_name: string }>(
    'SELECT tier_name FROM user_tier WHERE id = 1',
  );
...
export async function setUserTier(
  ...
  await getDb().runAsync(
    'UPDATE user_tier SET tier_name = ?, expires_at = ?, is_lifetime = ? WHERE id = 1',
    [tierName, expiresAt, isLifetime ? 1 : 0],
  );
}
